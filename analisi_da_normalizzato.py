...
st.write("Il file Excel viene caricato automaticamente (se configurato) oppure può essere caricato manualmente.")

# -------------------------------------------------------
# Lettura configurazione
# -------------------------------------------------------
MODE = st.secrets.get("MODE", "LOCAL")  # LOCAL | URL | UPLOAD
FILE_PATTERN = st.secrets.get("FILE_PATTERN", "*.xlsx")

st.sidebar.subheader("⚙️ Config")
st.sidebar.write("Modalità:", MODE)

def load_from_local():
    base_dir = st.secrets.get("LOCAL_BASE_DIR", None)
    if not base_dir:
        st.error("`LOCAL_BASE_DIR` non impostato nei secrets. Impostalo oppure cambia modalità.")
        st.stop()
    cartella = Path(base_dir)

    st.caption(f"📁 Percorso locale in uso: `{cartella}`")
    if not cartella.exists():
        st.error(f"❌ La cartella non esiste su questa macchina: {cartella}")
        st.stop()

    files = sorted(list(cartella.glob(FILE_PATTERN)), key=lambda p: p.stat().st_mtime, reverse=True)
    if not files:
        st.error(f"❌ Nessun file che combaci con `{FILE_PATTERN}` in `{cartella}`")
        st.stop()

    file_scelto = files[0]
    st.success(f"✅ Trovato file: {file_scelto.name}")
    df = pd.read_excel(file_scelto, engine="openpyxl")
    return df, str(file_scelto)

def load_from_url():
    FILE_URL = st.secrets.get("FILE_URL", None)
    if not FILE_URL:
        st.error("`FILE_URL` non impostato nei secrets. Impostalo oppure cambia modalità.")
        st.stop()

    st.caption(f"🔗 URL sorgente: {FILE_URL}")
    try:
        import requests  # su Streamlit Cloud è disponibile
        ...
        st.error(f"❌ Errore nello scaricare/leggere il file da URL: {e}")
        st.stop()

def load_from_upload():
    up = st.file_uploader("Carica un file Excel", type=["xlsx", "xls"])
    if not up:
        st.info("In attesa di upload…")
        st.stop()
    try:
...
# -------------------------------------------------------
# Caricamento dataset in base alla modalità
# -------------------------------------------------------
if MODE.upper() == "LOCAL":
    df, source = load_from_local()
elif MODE.upper() == "URL":
    df, source = load_from_url()
elif MODE.upper() == "UPLOAD":
    df, source = load_from_upload()
else:
    st.error("Modalità non riconosciuta. Usa: LOCAL | URL | UPLOAD")
    st.stop()

st.success(f"Origine dati: {source}")
